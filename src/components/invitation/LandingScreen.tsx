/**
 * LandingScreen.tsx — OPTIMISÉ PERFORMANCE + POLICES
 * ✅ MOBILE : polices beaucoup plus grandes (clamp min augmenté)
 * ✅ GRAND ÉCRAN : max values très généreuses
 * ✅ Stars CSS, trail throttlé, 6 particules
 */
      ...
      {chars.map((char, i) => (
        <motion.span
          ...
          style={{
            ...
            textShadow: litIndex === i ? "0 0 8px #fff, 0 0 24px rgba(255,230,100,0.95)" : "0 0 6px rgba(212,175,55,0.22)",
...
const SparkleFifty = ({ delay = 0 }: { delay?: number }) => (
  ...
  >
    <SparkleLine />
    <span style={{ fontFamily: FONTS.cinquante,
      /* ✅ MOBILE 1.4rem, GRAND ÉCRAN 2.2rem */
    ...
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none", overflow: "hidden" }}>
      ...
      <AnimatePresence>
        {shooting && (
          <motion.div key={shooting.id}
            ...
            initial={{ scaleX: 0, opacity: 0, x: 0 }} animate={{ scaleX: [0, 1, 0.2], opacity: [0, 1, 0], x: 220 }} exit={{ opacity: 0 }}
      ...
      {trails.map(t => (
        <motion.div key={t.id}
          ...
          initial={{ opacity: 0.7, scale: 1 }} animate={{ opacity: 0, scale: 0.1, y: -16 }} transition={{ duration: 0.65 }}
        />
      ))}
      ...
      <motion.div
        ...
        style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "clamp(300px, 60vw, 700px)", height: "clamp(300px, 60vw, 700px)", background: "radial-gradient(circle, rgba(212,175,55,0.06) 0%, transparent 65%)", borderRadius: "50%", filter: "blur(40px)", willChange: "transform, opacity" }}
      />
    </div>
  );
};
...
// ─── EXIT ANIMATION ───────────────────────────────────────────────────────────
const ExitRings = () => (
  <motion.div style={{ position: "fixed", inset: 0, zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }} initial={{ opacity: 1 }} animate={{ opacity: 1 }}>
    {[0, 1, 2, 3].map(i => (
      <motion.div key={i}
        style={{ position: "absolute", borderRadius: "50%", border: `${2 - i * 0.4}px solid rgba(212,175,55,${0.7 - i * 0.15})` }}
        initial={{ width: 60, height: 60, opacity: 1 }} animate={{ width: 900, height: 900, opacity: 0 }}
        transition={{ delay: i * 0.13, duration: 1.1, ease: "easeOut" }}
      />
    ))}
    <motion.div style={{ position: "absolute", inset: 0, backgroundColor: "rgba(212,175,55,0.12)" }} initial={{ opacity: 0 }} animate={{ opacity: [0, 0.6, 0] }} transition={{ duration: 0.7 }} />
  </motion.div>
);
...
const SparkleButton = ({ onEnter, visible }: { onEnter: () => void; visible: boolean }) => {
  ...
  return (
    <motion.div initial={{ x: 140, opacity: 0 }} animate={visible ? { x: 0, opacity: 1 } : { x: 140, opacity: 0 }} transition={{ type: "spring", stiffness: 75, damping: 14 }}>
      <motion.button
        onClick={onEnter}
        ...
        style={{
          position: "relative",
          /* ✅ MOBILE: padding plus grand */
          padding: "clamp(1rem, 2.5vw, 1.4rem) clamp(2.2rem, 4vw, 3.5rem)",
          background: "linear-gradient(135deg, #b8920e, #f0c842, #d4af37, #f0c842, #b8920e)",
          backgroundSize: "300% 100%", borderRadius: 100, border: "none", cursor: "pointer", overflow: "hidden",
      ...
      >
        <motion.div animate={{ x: ["-160%", "160%"] }} transition={{ repeat: Infinity, duration: 2.5, ease: "linear" }}
        ...
        <span style={{ position: "relative", zIndex: 1, fontFamily: FONTS.bouton,
          /* ✅ MOBILE: 1.3rem min, GRAND ÉCRAN: 2rem */
          fontSize: "clamp(1.3rem, 2.8vw, 2rem)",
          color: "#1a0f00", fontWeight: "bold", letterSpacing: "0.03em", display: "flex", alignItems: "center", gap: "0.55rem" }}>
          Entrer dans la fête
    ...
    <>
      ...
      >
        ...
        <div style={{ position: "relative", zIndex: 10, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center",
          /* ✅ GRAND ÉCRAN: padding latéral plus généreux */
          padding: "0 clamp(1.25rem, 5vw, 6rem)",
          width: "100%", gap: "0.8rem",
          /* ✅ GRAND ÉCRAN: limiter la largeur du contenu */
          maxWidth: "clamp(320px, 90vw, 1200px)", margin: "0 auto" }}>

          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3, delay: 0.1 }}>
            <BouncingLightText text="Événement Prestigieux" fontFamily={FONTS.evenement}
          ...
          <div>
            <SandTypewriter text="Joyeux Anniversaire" fontFamily={FONTS.joyeux}
              /* ✅ MOBILE: 3rem min, GRAND ÉCRAN: 9rem */
              fontSize="clamp(3rem, 8.5vw, 9rem)"
              color="#ffffff" delay={D.joyeux} />
          </div>

          <div>
            <SandTypewriter text="Mama Sylvie" fontFamily={FONTS.mama}
              /* ✅ MOBILE: 2rem min, GRAND ÉCRAN: 5rem */
              fontSize="clamp(2rem, 5.5vw, 5rem)"
              color="rgba(212,175,55,0.92)" delay={D.mama} />
          </div>
        ...
        {/* Coins décoratifs */}
        <div style={{ position: "absolute", top: "clamp(1.2rem,3vw,2.5rem)", left: "clamp(1.2rem,3vw,2.5rem)", width: "clamp(3rem,6vw,6rem)", height: "clamp(3rem,6vw,6rem)", borderTop: "1px solid rgba(212,175,55,0.3)", borderLeft: "1px solid rgba(212,175,55,0.3)", borderRadius: "12px 0 0 0", pointerEvents: "none" }} />
        <div style={{ position: "absolute", bottom: "clamp(1.2rem,3vw,2.5rem)", right: "clamp(1.2rem,3vw,2.5rem)", width: "clamp(3rem,6vw,6rem)", height: "clamp(3rem,6vw,6rem)", borderBottom: "1px solid rgba(212,175,55,0.3)", borderRight: "1px solid rgba(212,175,55,0.3)", borderRadius: "0 0 12px 0", pointerEvents: "none" }} />
      </motion.div>
